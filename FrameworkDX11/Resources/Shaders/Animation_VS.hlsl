...
//code supported by https://github.com/jjuiddong/Introduction-to-3D-Game-Programming-With-DirectX11/tree/master/Chapter%2025%20Character%20Animation/SkinnedMesh
...
PS_INPUT VS(VS_INPUT input)
{
	...
	//get position of vertex accouding to bone
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		posL += weights[i] *  mul(float4(input.Pos.xyz,1.0f), gBoneTransforms[input.BoneIndices[i]]).xyz;
		normalL += weights[i] * mul(input.Norm, (float3x3)gBoneTransforms[input.BoneIndices[i]]);
		tangentL += weights[i] * mul(input.Tangent.xyz, (float3x3)gBoneTransforms[input.BoneIndices[i]]);
	}
