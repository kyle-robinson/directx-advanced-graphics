...
LightingResult ComputeLighting( float4 vertexPos, float3 N, float3 vertexToEye, float3x3 TBN_inv, float4 shadowPos[MAX_LIGHTS] )
{
...
{
	float3 vertexToEye = normalize( EyePosition - input.WorldPos ).xyz;
    float3 vertexToEyeTS = mul( vertexToEye, input.TBN_inv );

	float2 parallaxTexCoords = ParallaxOcclusionMapping( input.Tex, vertexToEyeTS, input.Norm );
	if ( parallaxTexCoords.x > 1.0 || parallaxTexCoords.y > 1.0 || parallaxTexCoords.x < 0.0 || parallaxTexCoords.y < 0.0 )
		discard;

	float3 bumpMap = txNormal.Sample( smpLinear, parallaxTexCoords );
	bumpMap = ( bumpMap * 2.0f ) - 1.0f;
    bumpMap = normalize( mul( bumpMap, input.TBN ) );
	LightingResult lit = ComputeLighting( input.WorldPos, (float3)bumpMap, vertexToEye, input.TBN, input.LightViewPosition );
	...
	for ( int i = 0; i < MAX_LIGHTS; ++i )
	{
		float3 vertexToLight = normalize( Lights[i].Position - input.WorldPos ).xyz;
		float shadow = ParallaxSelfShadowing( vertexToLight, parallaxTexCoords, -input.Norm );
