...
LightingResult ComputeLighting( float4 vertexPos, float3 vertexToEye, float3 N, float4 shadowPos[MAX_LIGHTS] )
{
...
float4 PS( PS_INPUT input ) : SV_TARGET
{
	...
	float3 bumpMap = txNormal.Sample( smpLinear, parallaxTexCoords ).rgb;
	bumpMap = ( bumpMap * 2.0f ) - 1.0f;
    bumpMap = normalize( mul( bumpMap, input.TBN ) );
	LightingResult lit = ComputeLighting( input.WorldPos, vertexToEye, bumpMap, input.LightViewPosition );
