...
{
	float4 bumpMap = txNormal.Sample( smpLinear, input.Tex );
	bumpMap = ( bumpMap * 2.0f ) - 1.0f;
	float3 bumpedNorm = mul( (float1x3)bumpMap, input.TBN );
	LightingResult lit = ComputeLighting( input.WorldPos, bumpedNorm, input.LightViewPosition );
